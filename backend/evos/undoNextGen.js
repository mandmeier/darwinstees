...
export const undoNextGen = async (lineage, mutate, draw) => {
    ...
    try {
        const data = fs.readFileSync(evodir, 'utf8')
        const evos = JSON.parse(data)
        const generation = evos.length
        console.log(evos.length)
      } catch (err) {
        console.error(err)
    }
    ...
    //1) remove last evo from evos.js
    //2) replace mutant.js with ids from previous generation
    //3) delete last generation of mutants ONLY IF any faulty mutants were created
    //4) remove any designs with latest parent, will be overwritten with new one.


    


    // delete mutants by id



    //   try {
    //     const data = fs.readFileSync(mutantdir, 'utf8')
    //     const mutants = JSON.parse(data)
    //     console.log(mutants)
    //   } catch (err) {
    //     console.error(err)
